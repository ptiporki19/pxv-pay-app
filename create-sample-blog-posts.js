...
async function createSamplePosts() {
  try {
    ...
    const postsToCreate = samplePosts.filter(post => !existingSlugs.has(post.slug))

    if (postsToCreate.length === 0) {
      console.log('All sample posts already exist!')
      return
    }

    // Add the author_id to each post
    const postsWithAuthor = postsToCreate.map(post => ({
      ...post,
    ...
    console.log(`Successfully created ${data.length} sample blog posts:`)
    data.forEach(post => {
